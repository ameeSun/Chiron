        ...
        AF.upload(
            multipartFormData: { multipartFormData in
                if let spiral_data = image.pngData() {
                    multipartFormData.append(spiral_data, withName: "file", fileName: "wave.png", mimeType: "image/png")
                }
            ...
            to: "https://qtechsolutions.net/pd/api/testWave", method: .post)
...
struct PDWaveResultView_Previews: PreviewProvider {
    static var previews: some View {
        @State  var path: NavigationPath=NavigationPath()
        PDWaveResultView(path: $path, canvasView: PKCanvasView())
    }
}
